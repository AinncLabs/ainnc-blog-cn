...
## 2026-07-11 自然段与干货结构调整

- 旧规则导致自然段中位数只有 3 段、单段中位长度约 516 字；现已撤销“每段至少 400 字”。
- 71 篇文章全部重排，修改后每篇最少 8 个自然段，中位数 12 段，单段中位长度约 154 字。
- 新增 `contentType` 分类和差异化构建检查；干货/对比文章必须提供真正可用的表格、清单、示例或流程辅助。
- 清理通用开场标题、通用模块标题和批量重复的 TL;DR/过渡句，避免自然段增加以后仍然呈现流水线结构。
